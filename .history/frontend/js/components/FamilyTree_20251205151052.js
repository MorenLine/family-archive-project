const FamilyTree = {
    ...
    methods: {
        ...
        async fetchTreeData() {
            try {
                console.log('🔄 Получение данных для дерева...');

                // Определяем, какой endpoint использовать в зависимости от типа дерева
                let endpoint;
                switch (this.treeType) {
                    case 'ancestors':
                        endpoint = `http://localhost:8080/api/persons/${this.selectedRootPerson.id}/ancestors`;
                        break;
                    case 'descendants':
                        endpoint = `http://localhost:8080/api/persons/${this.selectedRootPerson.id}/descendants`;
                        break;
                    case 'both':
                        endpoint = `http://localhost:8080/api/persons/${this.selectedRootPerson.id}/tree`;
                        break;
                    default:
                        endpoint = `http://localhost:8080/api/persons/${this.selectedRootPerson.id}/tree`;
                }
                ...
                // Получаем данные дерева с бэкенда
                const response = await axios.get(endpoint);
                let treePersons = response.data;

                // Если нужно ограничить глубину, фильтруем данные
                if (this.treeType === 'both') {
                    // Для полного дерева ограничиваем глубину
                    treePersons = this.filterByDepth(treePersons, this.treeDepth);
                }

                console.log('✅ Получено персон из API:', treePersons.length);

                // Если данных мало, получаем дополнительно корневую персону
                if (treePersons.length === 0) {
                    const personResponse = await axios.get(`http://localhost:8080/api/persons/${this.selectedRootPerson.id}`);
                    treePersons = [personResponse.data];
                }
                ...
                // Если endpoint не найден, используем альтернативный метод
                console.log('⚠️ Используем альтернативный метод построения дерева...');
                return await this.fetchTreeDataAlternative();
            }
        },
