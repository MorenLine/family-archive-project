const FamilyTree = {
    ...
    methods: {
        ...
        createVisData(persons) {
            const nodes = [];
            // Используем Map для уникальных ребер по id
            const edgesMap = new Map();

            // Вычисляем пары супругов (по общим детям)
            const coupleChildren = new Map();
            persons.forEach(person => {
                // если у персоны есть оба родителя — регистрируем пару
                if (person.parent1 && person.parent2 && person.parent1.id && person.parent2.id) {
                    const p1 = person.parent1.id;
                    const p2 = person.parent2.id;
                    const key = p1 < p2 ? `${p1}-${p2}` : `${p2}-${p1}`;
                    if (!coupleChildren.has(key)) coupleChildren.set(key, new Set());
                    coupleChildren.get(key).add(person.id);
                }
            });
            ...
            persons.forEach(person => {
                ...
                // Добавляем связи: если у ребёнка есть оба родителя — привязка будет идти через marriage-узел
                if (person.parent1 && person.parent1.id && person.parent2 && person.parent2.id) {
                    const p1 = person.parent1.id;
                    const p2 = person.parent2.id;
                    const coupleKey = p1 < p2 ? `${p1}-${p2}` : `${p2}-${p1}`;
                    const marriageNodeId = `m-${coupleKey}`;

                    // создаём marriage-узел если ещё нет
                    if (!nodes.find(n => n.id === marriageNodeId)) {
                        nodes.push({
                            ...
                            shape: 'dot',
                            size: 8,
                            color: { background: '#444', border: '#444' },
                            physics: false,
                            font: { color: '#444' }
                        });
                    }

                    // связи родитель -> marriage (без стрелок)
                    const e1 = {
                        id: `${p1}->${marriageNodeId}`,
                        from: p1,
                        to: marriageNodeId,
                        arrows: '',
                        color: '#6c757d',
                    ...
                    edgesMap.set(e1.id, e1);

                    const e2 = {
                        id: `${p2}->${marriageNodeId}`,
                        from: p2,
                        to: marriageNodeId,
                        arrows: '',
                        color: '#6c757d',
                    ...
                    edgesMap.set(e2.id, e2);

                    // связь marriage -> child
                    const ec = {
                        id: `${marriageNodeId}->${person.id}`,
                        from: marriageNodeId,
                        to: person.id,
                        arrows: 'to',
                        color: '#007bff',
                    ...
                    edgesMap.set(ec.id, ec);

                } else {
                    // один из родителей или оба отсутствуют — обычные связи parent -> child
                    if (person.parent1 && person.parent1.id) {
