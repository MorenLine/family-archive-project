        ...
        <div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                                ...
                                <div v-for="(group, index) in timelineGroups" :key="group.year" class="timeline-group">
                                    <div class="timeline-year">
                                        ...
                                        <div class="timeline-photos">
                                            <div v-for="photo in group.photos" :key="photo.id" class="timeline-photo-item">
                                                <div class="card mb-2 photo-card d-flex flex-row" style="cursor:pointer;" @click="openLightbox(photo)">
                                                    <div class="card-body p-2" style="flex:1;">
                                                        <h6 class="card-title mb-1" style="font-size:0.95rem;">{{ photo.originalFileName || 'Без названия' }}</h6>
                                                        <p class="card-text mb-1 text-truncate" style="max-width:100%;">{{ photo.description || '' }}</p>
                                                        <small class="text-muted">{{ photo.photoDate ? formatDate(photo.photoDate) : 'Без даты' }}</small>
                                                    </div>
                                                    <div style="width:120px;display:flex;align-items:center;justify-content:center;padding:0.5rem;">
                                                        <img :src="getPhotoUrl(photo.fileName)" :alt="photo.description" style="width:100px;height:100px;object-fit:cover;border-radius:4px;" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div v-else-if="viewMode === 'grid' && photos.length > 0" class="grid-view">
                                <div class="row g-3">
                                    <div v-for="photo in photos" :key="photo.id" class="col-12 col-md-6 col-lg-4">
                                        <div class="card mb-3 photo-card d-flex flex-row" style="cursor:pointer;" @click="openLightbox(photo)">
                                            <div class="card-body p-3" style="flex:1;">
                                                <h6 class="card-title mb-1">{{ photo.originalFileName || 'Без названия' }}</h6>
                                                <p class="card-text mb-1 text-truncate">{{ photo.description || '' }}</p>
                                                <small class="text-muted">{{ photo.photoDate ? formatDate(photo.photoDate) : 'Без даты' }}</small>
                                            </div>
                                            <div style="width:160px;display:flex;align-items:center;justify-content:center;padding:0.5rem;">
                                                <img :src="getPhotoUrl(photo.fileName)" :alt="photo.description" style="width:140px;height:100px;object-fit:cover;border-radius:4px;" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div v-if="lightboxVisible" class="lightbox-overlay" @click="closeLightbox"
                ...
                <div class="lightbox-content" @click.stop
                     style="background:#0b0b0b;color:#fff;border-radius:8px;padding:1rem;max-width:95vw;max-height:90vh;display:flex;flex-direction:column;gap:0.75rem;">
                    <button class="lightbox-close btn btn-sm btn-outline-light" @click="closeLightbox" style="align-self:flex-end;">
                        <i class="bi bi-x-lg"></i>
                    </button>
                    <div class="lightbox-image-container" style="flex:1;display:flex;align-items:center;justify-content:center;overflow:auto;">
                    ...
                    <div class="lightbox-info" style="text-align:left;">
                        <h6 v-if="currentPhoto && currentPhoto.description" style="margin:0 0 0.25rem 0">{{ currentPhoto.description }}</h6>
                        <p v-if="currentPhoto && currentPhoto.photoDate" class="text-muted mb-0" style="margin:0">
                            {{ formatDate(currentPhoto.photoDate) }}
                        </p>
                    </div>
                    <div class="lightbox-navigation" style="margin-top:auto;display:flex;align-items:center;justify-content:center;gap:1rem;padding-top:0.5rem;">
