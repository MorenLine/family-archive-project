    ...
    methods: {
        ...
        applyLayoutSettings() {
            ...
            try {
                ...
                // ДЕБАГ: выводим полную информацию о связях
                console.log('=== ПОЛНАЯ ИНФОРМАЦИЯ О ДЕРЕВЕ ===');
                console.log('Узлы:', this.treeData.nodes?.length || 0);
                console.log('Связи:', this.treeData.edges?.length || 0);

                if (this.treeData.edges && this.treeData.edges.length > 0) {
                    console.log('--- ДЕТАЛИ СВЯЗЕЙ ---');
                    this.treeData.edges.forEach((edge, index) => {
                        const fromNode = this.treeData.nodes.find(n => n.id === edge.from);
                        const toNode = this.treeData.nodes.find(n => n.id === edge.to);
                        console.log(`Связь ${index + 1}:`);
                        console.log(`  От: ${fromNode?.label || 'ID:' + edge.from} (${edge.from})`);
                        console.log(`  К: ${toNode?.label || 'ID:' + edge.to} (${edge.to})`);
                        console.log(`  Тип: ${fromNode?.data?.gender || '?'} → ${toNode?.data?.gender || '?'}`);
                    });
                }
                console.log('==========================');
                ...
                if (this.treeData.nodes && this.treeData.nodes.length > 0) {
                    const nodesArray = this.treeData.nodes.map(node => {
                        // ОТЛАДОЧНЫЙ ВЫВОД
                        console.log('Узел:', node.id, 'данные:', node.data, 'пол:', node.data?.gender);

                        const gender = node.data?.gender || 'UNKNOWN';
                        const color = this.getNodeColor(gender);

                        console.log('Цвет для узла', node.id, ':', color);
        ...
        getNodeColor(gender) {
            ...
            // Приводим к строке и нормализуем значение
            let genderStr = '';

            if (typeof gender === 'string') {
                genderStr = gender.toUpperCase();
            } else if (gender && typeof gender === 'object') {
                // Если gender - объект enum, пробуем получить строковое значение
                genderStr = String(gender).toUpperCase();
            }

            console.log('Определение цвета для пола:', gender, '->', genderStr);

            if (genderStr.includes('MALE') || genderStr === 'M' || genderStr === 'МУЖСКОЙ') {
                return {
                    background: '#d4edda',
                    border: '#28a745',
            ...
            if (genderStr.includes('FEMALE') || genderStr === 'F' || genderStr === 'ЖЕНСКИЙ') {
                return {
                    background: '#f8d7da',
                    border: '#dc3545',
        ...
        async loadPersonDetails(personId) {
            try {
                ...
                // Получаем родителей напрямую из данных персоны
                const parents = [];
                if (personData.parent1) {
                    parents.push({
                        id: personData.parent1.id,
                        firstName: personData.parent1.firstName,
                        lastName: personData.parent1.lastName,
                        gender: personData.parent1.gender
                    });
                }
                if (personData.parent2) {
                    parents.push({
                        id: personData.parent2.id,
                        firstName: personData.parent2.firstName,
                        lastName: personData.parent2.lastName,
                        gender: personData.parent2.gender
                    });
                }

                // Получаем детей через API
                let children = [];
                try {
                    ...
                    console.log('API детей не доступен, используем данные дерева');
                    // Используем данные из treeData как fallback
                ...
                console.log('Загружена персона:', this.selectedPerson);

            } catch (error) {
                console.error('Ошибка загрузки деталей персоны:', error);
                // Используем данные из списка и treeData как fallback
                const personInList = this.allPersons.find(p => p.id === personId);
                if (personInList) {
                    // Определяем родителей и детей из treeData
                    let parents = [];
                    let children = [];

                    if (this.treeData && this.treeData.edges) {
                        // В treeData: from → to означает from является родителем to
                        // Значит родители текущей персоны - это те, у кого edge.to === personId
                        ...
                        // Дети текущей персоны - это те, у кого edge.from === personId
                        children = this.treeData.edges
                            .filter(edge => edge.from === personId)
                            .map(edge => {
                                const node = this.treeData.nodes.find(n => n.id === edge.to);
        ...
        calculateSiblingsCount(personId, parents) {
            ...
            // Находим всех, у кого такие же родители
            if (this.treeData.edges) {
                // Собираем всех детей общих родителей
                const allChildren = [];

                // Для каждого родителя находим всех его детей
                parentIds.forEach(parentId => {
                    const childrenOfParent = this.treeData.edges
                        .filter(edge => edge.from === parentId)
                        .map(edge => edge.to);
                    allChildren.push(...childrenOfParent);
                });

                // Убираем дубликаты и исключаем саму персону
                const uniqueChildren = [...new Set(allChildren)];
                siblings = uniqueChildren.filter(childId => childId !== personId);
            }
        ...
        getGenderDisplay(gender) {
            ...
            if (genderStr.includes('MALE') || genderStr.includes('МУЖ')) {
                return 'Мужской';
            }

            if (genderStr.includes('FEMALE') || genderStr.includes('ЖЕН')) {
                return 'Женский';
            }
