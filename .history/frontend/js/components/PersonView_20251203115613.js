const PersonView = {
    template: `
        <div>
            ...
            <!-- Модал редактирования метаданных фото -->
            <div v-if="editModalVisible" class="modal d-block" tabindex="-1" @click="cancelEdit">
                <div class="modal-dialog" @click.stop>
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Редактировать фото</h5>
                            <button type="button" class="btn-close" aria-label="Close" @click="cancelEdit"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Название (имя файла)</label>
                                <input class="form-control" v-model="editPhoto.originalFileName" maxlength="255" />
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Описание</label>
                                <textarea class="form-control" v-model="editPhoto.description" rows="3"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" @click="cancelEdit">Отмена</button>
                            <button type="button" class="btn btn-primary" @click="savePhotoMetadata">Сохранить</button>
                        </div>
                    </div>
                </div>
            </div>
