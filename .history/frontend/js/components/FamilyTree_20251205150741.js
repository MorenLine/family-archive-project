const FamilyTree = {
    ...
    methods: {
        ...
        async fetchTreeData() {
            ...
            const fetchTreeDataRecursive = async (personId, currentDepth, direction = 'both') => {
                ...
                try {
                    ...
                    // Получаем родителей (предки)
                    if ((this.treeType === 'ancestors' || this.treeType === 'both') && currentDepth < this.treeDepth - 1) {
                        if (person.parent1) {
                            await fetchTreeDataRecursive(person.parent1.id, currentDepth + 1, 'up');
                        }
                        if (person.parent2) {
                            await fetchTreeDataRecursive(person.parent2.id, currentDepth + 1, 'up');
                        }
                    }

                    // Получаем детей (потомки)
                    if ((this.treeType === 'descendants' || this.treeType === 'both') && currentDepth < this.treeDepth - 1) {
                        // Используем endpoint для получения детей
                        try {
                            const childrenResponse = await axios.get(`http://localhost:8080/api/persons/${personId}/children`);
                            const children = childrenResponse.data;

                            for (const child of children) {
                                await fetchTreeDataRecursive(child.id, currentDepth + 1, 'down');
                            }
                        } catch (error) {
                            console.warn(`Не удалось получить детей для персоны ${personId}:`, error.message);
                        }
                    }
            ...
            await fetchTreeDataRecursive(this.selectedRootPerson.id, 0);
