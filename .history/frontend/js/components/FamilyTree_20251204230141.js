            ...
            try {
                ...
                if (this.treeData.nodes && this.treeData.nodes.length > 0) {
                    const nodesArray = this.treeData.nodes.map(node => {
                        // Получаем пол из данных
                        let genderValue = node.data?.gender || node.gender || 'UNKNOWN';

                        // Если gender - объект enum, берем его строковое значение
                        ...
                        return {
                            ...
                            color: this.getNodeColor(genderValue),
                            font: { size: 12 },
                            data: {
                                ...(node.data || {}),
                                gender: genderValue
