const FamilyTree = {
    ...
    methods: {
        ...
        applyLayoutSettings() {
            ...
            try {
                console.log('Данные дерева:', this.treeData); // ОТЛАДКА
                ...
                if (this.treeData.nodes && this.treeData.nodes.length > 0) {
                    console.log('Первый узел для отладки:', this.treeData.nodes[0]); // ОТЛАДКА

                    const nodesArray = this.treeData.nodes.map(node => {
                        // Получаем пол из данных - исправленный вариант
                        let genderValue = null;

                        // Проверяем разные варианты расположения данных
                        if (node.data && node.data.gender) {
                            genderValue = node.data.gender;
                        } else if (node.gender) {
                            genderValue = node.gender;
                        }

                        console.log(`Узел ${node.id}, gender:`, genderValue, typeof genderValue); // ОТЛАДКА

                        // Преобразуем к строке
                        const genderStr = this.normalizeGender(genderValue);

                        return {
                            id: node.id,
                            label: node.label || `${node.data?.firstName || ''} ${node.data?.lastName || ''}`.trim() || 'Без имени',
                            title: this.createTooltip(node),
                            color: this.getNodeColor(genderStr),
                            font: { size: 12 },
                            data: {
                                ...(node.data || {}),
                                gender: genderStr
        ...
        normalizeGender(gender) {
            ...
            // Если это объект (enum)
            if (typeof gender === 'object') {
                if (gender.toString) {
                    const str = gender.toString().toUpperCase();
        ...
        async loadPersonDetails(personId) {
            try {
                ...
                let genderValue = personData.gender;
                if (genderValue && typeof genderValue === 'object') {
                    genderValue = genderValue.toString ? genderValue.toString() : 'UNKNOWN';
                }
                ...
                this.selectedPerson = {
                    ...
                    gender: genderValue, // Используем обработанное значение
        ...
        getGenderIcon(gender) {
            const genderStr = this.normalizeGender(gender);

            switch (genderStr) {
                case 'MALE': return 'bi bi-gender-male';
                case 'FEMALE': return 'bi bi-gender-female';
                default: return 'bi bi-gender-ambiguous';
            }
        },
