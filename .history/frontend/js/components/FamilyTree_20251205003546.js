    ...
    methods: {
        ...
        async loadTree() {
            ...
            try {
                ...
                console.log('Данные дерева:', this.treeData);
        ...
        createNetwork() {
            ...
            try {
                ...
                const nodes = new vis.DataSet(this.treeData.nodes.map(node => {
                    ...
                    // Определяем пол для цвета
                    const gender = node.data?.gender || node.gender;
                    console.log(`Узел ${node.id}, пол: ${gender}`);
                    ...
                    if (gender === 'MALE') {
                        ...
                        processedNode.font = { color: '#0f5132' };
                        ...
                        processedNode.font = { color: '#721c24' };
                    } else {
                        processedNode.color = {
                            background: '#e2e3e5',
                            border: '#6c757d',
                        ...
                        processedNode.font = { color: '#495057' };
                    }
                    ...
                    if (node.data?.birthDate) {
                        try {
                            const birthYear = new Date(node.data.birthDate).getFullYear();
                            label += `\\n(${birthYear})`;
                        } catch (e) {
                            console.warn('Не удалось получить год рождения для узла', node.id);
                        }
                    }
                    ...
                    processedNode.widthConstraint = { minimum: 120, maximum: 160 };
        ...
        async loadPersonInfo(personId) {
            try {
                // Пробуем загрузить детальную информацию
                ...
                console.warn('Не удалось загрузить детальную информацию, используем базовые данные');

                // Используем данные из дерева или из карты всех персон
                let person = null;

                // Ищем в данных дерева
                if (this.treeData && this.treeData.nodes) {
                    const node = this.treeData.nodes.find(n => n.id === personId);
                    if (node && node.data) {
                        person = node.data;
                    }
                }

                // Ищем в карте всех персон
                if (!person && this.allPersonsMap) {
                    person = this.allPersonsMap.get(personId);
                }
