                                        ...
                                        <div class="card photo-card" :class="{ 'border-primary': photo.isMain }">
                                            ...
                                            <div class="card-body p-2">
                                                <div class="d-flex justify-content-between align-items-start">
                                                                    <div class="photo-info flex-grow-1 me-2">
                                                                        <small class="text-muted d-block">
                                                                            {{ photo.photoDate ? formatDate(photo.photoDate) : 'Без даты' }}
                                                                        </small>

                                                                        <!-- Просмотр или редактирование метаданных -->
                                                                        <div v-if="photo.editing">
                                                                            <div class="mb-1">
                                                                                <input v-model="photo.editName" 
                                                                                       type="text" 
                                                                                       class="form-control form-control-sm" 
                                                                                       placeholder="Название фото (ориг. имя)">
                                                                            </div>
                                                                            <div class="mb-1">
                                                                                <input v-model="photo.editDescription" 
                                                                                       type="text" 
                                                                                       class="form-control form-control-sm" 
                                                                                       placeholder="Описание фото">
                                                                            </div>
                                                                            <div class="d-flex gap-1">
                                                                                <button @click="savePhotoMetadata(photo)" 
                                                                                        class="btn btn-success btn-sm">Сохранить</button>
                                                                                <button @click="cancelEditPhoto(photo)" 
                                                                                        class="btn btn-outline-secondary btn-sm">Отмена</button>
                                                                            </div>
                                                                        </div>

                                                                        <div v-else>
                                                                            <small v-if="photo.originalFileName" 
    ...
    methods: {
        ...
        async loadPhotos() {
            try {
                ...
                // Инициализируем поля для редактирования на фронте
                this.photos = response.data.map(p => ({
                    ...p,
                    editing: false,
                    editName: p.originalFileName || '',
        ...
        editPhoto(photo) {
            // Включаем режим редактирования для одной фотографии
            this.photos.forEach(p => { if (p.id !== photo.id) p.editing = false; });
            photo.editing = true;
            // Синхроним поля на случай если локальные не изменялись
            photo.editName = photo.originalFileName || '';
            photo.editDescription = photo.description || '';
        },

        cancelEditPhoto(photo) {
            photo.editing = false;
            photo.editName = photo.originalFileName || '';
            photo.editDescription = photo.description || '';
        },
        ...
        async savePhotoMetadata(photo) {
            ...
            try {
                const dto = {
                    ...
                    originalFileName: photo.editName,
                    description: photo.editDescription,
                    photoDate: photo.photoDate || null,
                    sortOrder: photo.sortOrder || null,
                    personId: this.$route.params.id
                };

                await axios.put(`http://localhost:8080/api/photos/${photo.id}`, dto);
                // Обновляем локальную модель
                photo.originalFileName = photo.editName;
                photo.description = photo.editDescription;
                photo.editing = false;
                this.successMessage = 'Данные фотографии сохранены';
            } catch (error) {
                console.error('Ошибка сохранения метаданных фото:', error);
                this.errorMessage = 'Ошибка при сохранении метаданных фото: ' + (error.response?.data || error.message);
            }
        },
