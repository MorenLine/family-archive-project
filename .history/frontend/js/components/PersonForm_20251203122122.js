const PersonForm = {
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <form @submit.prevent="submitForm">
                                ...
                                <!-- Модальное окно для добавления/редактирования фото -->
                                <div v-if="photoModalVisible" class="modal-overlay" @click.self="closePhotoModal"
                                     style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:1050;background:rgba(0,0,0,0.5);">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex flex-column flex-md-row">
                                                <div class="me-3" style="flex:1;display:flex;align-items:center;justify-content:center;">
                                                    <img :src="photoModalImage" alt="Фото" style="max-width:100%; max-height:80vh; object-fit:contain;" />
                                                </div>
                                                <div style="width:320px;">
                                                    <h5 v-if="photoModalMode === 'edit'">Редактировать фото</h5>
                                                    <h5 v-else>Добавление фото</h5>
                                                    <div class="mb-2"><label class="form-label small">Название</label>
                                                        <input v-model="photoModalData.originalFileName" class="form-control form-control-sm" />
                                                    </div>
                                                    <div class="mb-2"><label class="form-label small">Описание</label>
                                                        <input v-model="photoModalData.description" class="form-control form-control-sm" />
                                                    </div>
                                                    <div class="mb-2"><label class="form-label small">Дата фото</label>
                                                        <input v-model="photoModalData.photoDate" type="date" class="form-control form-control-sm" :max="today" />
                                                    </div>
                                                    <div class="d-flex justify-content-end gap-2 mt-3">
                                                        <button class="btn btn-secondary" @click="closePhotoModal">Отмена</button>
                                                        <button class="btn btn-primary" @click="savePhotoFromModal">Сохранить</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div v-if="isEditMode" class="mb-3">
                                ...
                                <div v-if="newPhoto" class="mt-3 p-3 border rounded">
                                    <div class="row align-items-center">
                                        <div class="col-auto">
                                            <img :src="newPhoto.preview" 
                                                 class="img-thumbnail" 
                                        ...
                                        <div class="col">
                                            <div class="mb-2">
                                                ...
                                                <input v-model="newPhoto.description" 
                                                       type="text" 
                                            ...
                                            <div>
                                                ...
                                                <input v-model="newPhoto.photoDate" 
                                                       type="date" 
                                                       class="form-control form-control-sm"
                                                       :max="today">
                                            </div>
                                        ...
                                        <div class="col-auto">
                                            <button @click="openPhotoModalForUpload" 
                                                    class="btn btn-success btn-sm"
                                                    :disabled="uploadingPhoto">
                                                <span v-if="uploadingPhoto" class="spinner-border spinner-border-sm"></span>
                                                                ...
                                                                <div class="photo-actions d-flex justify-content-center gap-1 mt-2">
                                                                    <div class="btn-group" role="group" aria-label="photo-actions">
                                                                        ...
                                                                        <button v-else
                                                                            ...
                                                                            <button @click="openPhotoModalToEdit(photo)" 
    ...
    data() {
        return {
            ...
            photoModalMode: '', // 'upload' or 'edit'
            photoModalImage: null,
            photoModalFile: null,
            photoModalPhotoId: null,
            photoModalData: {
                originalFileName: '',
                description: '',
                photoDate: ''
    ...
    methods: {
        ...
        // Открыть модальное окно для загрузки (используется кнопкой в блоке newPhoto)
        openPhotoModalForUpload() {
            if (!this.newPhoto) return;
            this.photoModalMode = 'upload';
            this.photoModalImage = this.newPhoto.preview;
            this.photoModalFile = this.newPhoto.file;
            this.photoModalPhotoId = null;
            this.photoModalData = {
                originalFileName: this.newPhoto.file.name || '',
                description: this.newPhoto.description || '',
                photoDate: this.newPhoto.photoDate || ''
            };
            this.photoModalVisible = true;
        },

        // Открыть модальное окно для редактирования существующего фото
        openPhotoModalToEdit(photo) {
            this.photoModalMode = 'edit';
            this.photoModalImage = this.getPhotoUrl(photo.fileName);
            this.photoModalFile = null;
            this.photoModalPhotoId = photo.id;
            this.photoModalData = {
        ...
        closePhotoModal() {
            ...
            this.photoModalMode = '';
            this.photoModalImage = null;
            this.photoModalFile = null;
            this.photoModalPhotoId = null;
            this.photoModalData = { originalFileName: '', description: '', photoDate: '' };
        },

        // Сохранить данные из модального окна: загрузка нового фото или обновление метаданных
        async savePhotoFromModal() {
            try {
                if (this.photoModalMode === 'upload') {
                    if (!this.photoModalFile) return;
                    const formData = new FormData();
                    formData.append('file', this.photoModalFile);
                    formData.append('personId', this.$route.params.id);
                    formData.append('description', this.photoModalData.description || '');
                    if (this.photoModalData.photoDate) formData.append('photoDate', this.photoModalData.photoDate);
                    await axios.post('http://localhost:8080/api/photos/upload', formData, {
                ...
                } else if (this.photoModalMode === 'edit') {
                    if (!this.photoModalPhotoId) return;
                    const dto = {
                        id: this.photoModalPhotoId,
                        originalFileName: this.photoModalData.originalFileName,
                        description: this.photoModalData.description,
                        photoDate: this.photoModalData.photoDate || null
                    };
                    await axios.put(`http://localhost:8080/api/photos/${this.photoModalPhotoId}`, dto);
                ...
                console.error('❌ Ошибка при сохранении фото из модального окна:', error);
                this.errorMessage = 'Ошибка при сохранении: ' + (error.response?.data || error.message);
            }
        },
