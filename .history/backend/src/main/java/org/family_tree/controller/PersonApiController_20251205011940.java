...
public class PersonApiController {
    ...
    private Map<String, Object> buildTreeWithSiblings(Person rootPerson, int maxDepth, boolean includeSiblings) {
        ...
        // Фаза 1: Идём вверх к родителям
        Queue<Object[]> upQueue = new LinkedList<>();
        upQueue.offer(new Object[] { rootPerson, 0 });

        while (!upQueue.isEmpty()) {
            Object[] item = upQueue.poll();
            Person person = (Person) item[0];
            int currentDepth = (int) item[1];

            if (person == null || currentDepth > maxDepth || visited.contains(person.getId())) {
                continue;
            }
            ...
            // Добавляем узел
            Map<String, Object> node = createEnhancedNode(person);
            nodes.add(node);

            // Если не достигли максимальной глубины - идём к родителям
            if (currentDepth < maxDepth) {
                if (person.getParent1() != null && !visited.contains(person.getParent1().getId())) {
                    upQueue.offer(new Object[] { person.getParent1(), currentDepth + 1 });
                    // Стрелка от родителя к ребёнку
                    addEdgeIfNotExists(edges, person.getParent1().getId(), person.getId(), "parent");
                }

                if (person.getParent2() != null && !visited.contains(person.getParent2().getId())) {
                    upQueue.offer(new Object[] { person.getParent2(), currentDepth + 1 });
                    // Стрелка от родителя к ребёнку
                    addEdgeIfNotExists(edges, person.getParent2().getId(), person.getId(), "parent");
                }
            }
        }

        // Фаза 2: Идём вниз к детям от корневого элемента
        Queue<Object[]> downQueue = new LinkedList<>();
        downQueue.offer(new Object[] { rootPerson, 0 });
        Set<Long> visitedDown = new HashSet<>();
        visitedDown.add(rootPerson.getId());

        while (!downQueue.isEmpty()) {
            Object[] item = downQueue.poll();
            Person person = (Person) item[0];
            int currentDepth = (int) item[1];
            ...
            if (currentDepth < maxDepth) {
                ...
                for (Person child : children) {
                    if (!visitedDown.contains(child.getId())) {
                        visitedDown.add(child.getId());
                        downQueue.offer(new Object[] { child, currentDepth + 1 });

                        // Добавляем узел если его ещё нет
                        if (!nodes.stream().anyMatch(n -> n.get("id").equals(child.getId()))) {
                            Map<String, Object> childNode = createEnhancedNode(child);
                            nodes.add(childNode);
                        }

                        // Стрелка от родителя к ребёнку
                        addEdgeIfNotExists(edges, person.getId(), child.getId(), "child");
                    }
                }
            }

            // Добавляем братьев/сестер на том же уровне (если включено и это корневой узел)
            if (includeSiblings && currentDepth == 0) {
                List<Person> siblings = personService.findSiblings(person.getId());
                for (Person sibling : siblings) {
                    if (!visitedDown.contains(sibling.getId())) {
                        visitedDown.add(sibling.getId());
                        downQueue.offer(new Object[] { sibling, currentDepth });

                        // Добавляем узел если его ещё нет
                        if (!nodes.stream().anyMatch(n -> n.get("id").equals(sibling.getId()))) {
                            Map<String, Object> siblingNode = createEnhancedNode(sibling);
                            nodes.add(siblingNode);
                        }

                        // Связываем братьев/сестер: предпочтительно через общих родителей (родитель -> ребёнок),
                        // чтобы vis.js поместил братьев/сестер на одном уровне с персоной.
                        boolean parentConnected = false;
                        if (person.getParent1() != null) {
                            addEdgeIfNotExists(edges, person.getParent1().getId(), sibling.getId(), "parent");
                            parentConnected = true;
                        }
                        if (person.getParent2() != null) {
                            addEdgeIfNotExists(edges, person.getParent2().getId(), sibling.getId(), "parent");
                            parentConnected = true;
                        }
                        // Если родители неизвестны, делаем запасной связующий край типа "sibling"
                        if (!parentConnected) {
