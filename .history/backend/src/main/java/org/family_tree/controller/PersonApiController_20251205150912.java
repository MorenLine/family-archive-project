...
public class PersonApiController {
    ...
    @GetMapping("/persons/{id}/ancestors")
    public ResponseEntity<List<Person>> getAncestors(@PathVariable Long id) {
        List<Person> ancestors = personService.getAncestors(id);
        return ResponseEntity.ok(ancestors);
    }

    @GetMapping("/persons/{id}/descendants")
    public ResponseEntity<List<Person>> getDescendants(@PathVariable Long id) {
        List<Person> descendants = personService.getDescendants(id);
        return ResponseEntity.ok(descendants);
    }

    @GetMapping("/persons/{id}/tree")
    public ResponseEntity<List<Person>> getFamilyTree(@PathVariable Long id) {
        List<Person> tree = personService.getFamilyTree(id);
    ...
    public ResponseEntity<PersonService.ArchiveStatistics> getArchiveStatistics() {
        PersonService.ArchiveStatistics stats = personService.getArchiveStatistics();
