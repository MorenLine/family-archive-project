...
public class PersonApiController {
    ...
    public ResponseEntity<Person> updatePerson(@PathVariable Long id, @RequestBody Person personDetails) {
        ...
        // Обновляем родителей если они переданы
        if (personDetails.getParent1() != null && personDetails.getParent1().getId() != null) {
            Optional<Person> parent1 = personService.findById(personDetails.getParent1().getId());
            parent1.ifPresent(person::setParent1);
        } else {
            person.setParent1(null);
        }
        
        if (personDetails.getParent2() != null && personDetails.getParent2().getId() != null) {
            Optional<Person> parent2 = personService.findById(personDetails.getParent2().getId());
            parent2.ifPresent(person::setParent2);
        } else {
