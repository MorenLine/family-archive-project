...
public class PersonApiController {
    ...
    private Map<String, Object> buildTreeWithSiblings(Person rootPerson, int maxDepth, boolean includeSiblings) {
        ...
        // Фаза 1: Идём вверх к родителям и добавляем их детей (чтобы братья/сестры были связаны через родителя)
        ...
        while (!upQueue.isEmpty()) {
            ...
            // Добавляем узел, если ещё нет
            if (!nodes.stream().anyMatch(n -> n.get("id").equals(person.getId()))) {
                nodes.add(createEnhancedNode(person));
            }

            // Если не достигли максимальной глубины - идём к родителям
            if (currentDepth < maxDepth) {
                Person[] parents = new Person[] { person.getParent1(), person.getParent2() };
                for (Person parent : parents) {
                    if (parent != null) {
                        if (!visited.contains(parent.getId())) {
                            upQueue.offer(new Object[] { parent, currentDepth + 1 });
                        }

                        // Добавляем узел родителя
                        if (!nodes.stream().anyMatch(n -> n.get("id").equals(parent.getId()))) {
                            nodes.add(createEnhancedNode(parent));
                        }

                        // Стрелка от родителя к текущему (parent -> child)
                        addEdgeIfNotExists(edges, parent.getId(), person.getId(), "parent");

                        // Если включены братья/сестры - добавляем всех детей этого родителя
                        if (includeSiblings) {
                            List<Person> parentChildren = personService.findChildren(parent.getId());
                            for (Person child : parentChildren) {
                                if (!nodes.stream().anyMatch(n -> n.get("id").equals(child.getId()))) {
                                    nodes.add(createEnhancedNode(child));
                                }
                                // Создаём связь parent -> child (это гарантирует, что все дети одного родителя окажутся на одном уровне)
                                addEdgeIfNotExists(edges, parent.getId(), child.getId(), "child");
                            }
                        }
                    }
                }
        ...
        // Фаза 2: Идём вниз к детям от корневого элемента (обычный обход вниз)
        ...
        while (!downQueue.isEmpty()) {
            ...
            // Добавляем узел если ещё нет (на случай, если не добавлен на фазе вверх)
            if (!nodes.stream().anyMatch(n -> n.get("id").equals(person.getId()))) {
                nodes.add(createEnhancedNode(person));
            }
