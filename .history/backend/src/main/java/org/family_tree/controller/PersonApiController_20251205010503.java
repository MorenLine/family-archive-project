...
public class PersonApiController {
    ...
    private Map<String, Object> buildTreeWithSiblings(Person rootPerson, int maxDepth, boolean includeSiblings) {
        ...
        // Начинаем с корневой персоны и идем вниз к детям
        Queue<Object[]> queue = new LinkedList<>();
        queue.offer(new Object[] { rootPerson, 0 });
            ...
            if (currentDepth < maxDepth) {
                ...
                for (Person child : children) {
                    if (!visited.contains(child.getId())) {
                        queue.offer(new Object[] { child, currentDepth + 1 });
                        // Добавляем связь от родителя к ребенку
            ...
            if (includeSiblings && currentDepth == 0) {
                ...
                for (Person sibling : siblings) {
                    if (!visited.contains(sibling.getId())) {
                        queue.offer(new Object[] { sibling, currentDepth });
