...
public class PersonApiController {
    ...
    private Map<String, Object> buildTreeWithSiblings(Person rootPerson, int maxDepth, boolean includeSiblings) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (nodeDepths.get(person.getId()) != null && nodeDepths.get(person.getId()) < depth) {
                // Узел уже обработан на меньшей глубине
                if (parentId != null) {
            ...
            // Создаем или обновляем узел (и добавляем только если новый)
