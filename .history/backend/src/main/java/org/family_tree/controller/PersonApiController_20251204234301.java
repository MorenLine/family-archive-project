...
public class PersonApiController {
    ...
    private void collectTreeNodes(Person person, List<Map<String, Object>> nodes,
        ...
        if (person.getParent1() != null) {
            Map<String, Object> parentEdge = new HashMap<>();
            parentEdge.put("from", person.getParent1().getId()); // РОДИТЕЛЬ
            parentEdge.put("to", person.getId()); // РЕБЕНОК (текущая персона)
            parentEdge.put("arrows", "to");
        ...
        if (person.getParent2() != null) {
            Map<String, Object> parentEdge = new HashMap<>();
            parentEdge.put("from", person.getParent2().getId()); // РОДИТЕЛЬ
            parentEdge.put("to", person.getId()); // РЕБЕНОК (текущая персона)
            parentEdge.put("arrows", "to");
            edges.add(parentEdge);
        ...
        for (Person child : children) {
            // Для детей связь уже правильная: person (родитель) → child (ребенок)
            Map<String, Object> childEdge = new HashMap<>();
            childEdge.put("from", person.getId()); // РОДИТЕЛЬ (текущая персона)
            childEdge.put("to", child.getId()); // РЕБЕНОК
            childEdge.put("arrows", "to");
            edges.add(childEdge);
