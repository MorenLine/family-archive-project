...
public class PersonApiController {
    ...
    private Map<String, Object> buildTreeWithSiblings(Person rootPerson, int maxDepth, boolean includeSiblings) {
        ...
        while (!queue.isEmpty()) {
            ...
            // Добавляем связь
            if (parentId != null) {
                // Для всех типов отношений: от parentId к person.getId()
                addEdgeIfNotExists(edges, parentId, person.getId(), relationType);
            }
            ...
            if (includeSiblings && depth == 0) {
                ...
                for (Person sibling : siblings) {
                    if (!nodeDepths.containsKey(sibling.getId())) {
                        queue.offer(new Object[] { sibling, depth, person.getId(), "sibling" });
