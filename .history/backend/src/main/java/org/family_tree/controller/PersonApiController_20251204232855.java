...
public class PersonApiController {
    ...
    private void collectTreeNodes(Person person, List<Map<String, Object>> nodes,
        ...
        data.put("gender", person.getGender() != null ? person.getGender().name() : null);
        ...
        // ВАЖНО: Добавляем связь с родителем (если есть)
        // parentId → person.getId() означает: parentId является родителем
        // person.getId()
        if (parentId != null) {
            Map<String, Object> edge = new HashMap<>();
            edge.put("from", parentId); // РОДИТЕЛЬ
            edge.put("to", person.getId()); // РЕБЕНОК
            edge.put("arrows", "to");
            edges.add(edge);
        }
        ...
        if (person.getParent1() != null) {
            // person.getParent1() является родителем person
            // Рекурсивно обрабатываем родителя
            collectTreeNodes(person.getParent1(), nodes, edges, processedIds,
                    currentDepth + 1, maxDepth, person.getParent1().getId());

            // Добавляем связь parent1 → person
            Map<String, Object> edge = new HashMap<>();
            edge.put("from", person.getParent1().getId());
        ...
        if (person.getParent2() != null) {
            // person.getParent2() является родителем person
            // Рекурсивно обрабатываем родителя
            collectTreeNodes(person.getParent2(), nodes, edges, processedIds,
                    currentDepth + 1, maxDepth, person.getParent2().getId());

            // Добавляем связь parent2 → person
            Map<String, Object> edge = new HashMap<>();
            edge.put("from", person.getParent2().getId());
            edge.put("to", person.getId());
            edge.put("arrows", "to");
            edges.add(edge);
        }
        ...
        for (Person child : children) {
            // person является родителем child
            // Рекурсивно обрабатываем ребенка
            collectTreeNodes(child, nodes, edges, processedIds,
                    currentDepth + 1, maxDepth, person.getId());

            // Связь уже будет добавлена при обработке ребенка (см. условие if (parentId !=
            // null) выше)
            // Так как мы передаем person.getId() как parentId при рекурсивном вызове
        }
    }
